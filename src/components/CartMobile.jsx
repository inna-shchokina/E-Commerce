  ...
  return (
    <div className="lg:hidden flex flex-col justify-center items-center py-4 mt-6 space-y-6">
      {cartItems.map((item, index) => (
        <div
          key={index}
          className="flex flex-col border border-slate-200 py-2 w-64 h-96"
        >
          {/* Delete button and pic */}
          <div className="relative flex justify-center mb-4 mt-2">
            <button
              className="absolute -top-4 right-2 text-slate-800 text-xl p-2"
            ...
            <img
              src={item.image}
              alt={item.title}
              className="mt-8 w-24 h-24 object-contain"
            />
          </div>

          {/* Title and Description */}
          <div className="flex justify-center items-center mb-2 mt-4 px-4">
            <h3 className="text-base text-slate-800 font-medium text-center">
              {item.title}
            </h3>
          </div>
